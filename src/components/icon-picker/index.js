						...
						<PanelRow>
							<BaseControl>
								<ul className="gblocks-icon-chooser">
									{
										Object.keys( faSvgs ).map( ( svg, i ) => {
											return (
												<li key={ `editor-pblock-types-list-item-${ i }` }>
													<Tooltip text={ ( faSvgs[ svg ].label ) }>
														<Button
															isLarge
															className="editor-block-list-item-button"
															onClick={ () => {
																setAttributes( {
																	[ this.props[ 'attrIcon' ] ]: renderToString( faSvgs[ svg ][ 'icon' ] ), // eslint-disable-line dot-notation
																	'hasIcon': true, // eslint-disable-line quote-props
																} );
															} }
														>
